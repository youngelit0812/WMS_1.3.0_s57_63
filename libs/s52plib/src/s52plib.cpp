...
int s52plib::S52_load_Plib(const wxString &PLib, bool b_forceLegacy) {
  ...
  if (!m_chartSymbols.LoadConfigFile(this, PLib)) {
    printf("s57plib : Could not load XML PLib symbol file: %s\n", (const char *)PLib.mb_str(wxConvUTF8));    
  ...
  wxString s57data_dir = *(m_pPlatform->GetSharedDataDirPtr());
...
//#endif
    ...
    if (b_force_no_texfont) {
      if (!ptext->texobj){  // is texture ready?
        ...
        scaled_font = FontMgr::Get().FindOrCreateFont(new_size, ptext->pFont->GetFamily(), ptext->pFont->GetStyle(), ptext->pFont->GetWeight(), false, ptext->pFont->GetFaceName(), wxFONTENCODING_DEFAULT);
        wxScreenDC sdc;        
    ...
    if (scale_factor > 1) {
      ...
      wxFont *scaled_font = FontMgr::Get().FindOrCreateFont(new_size, pf->GetFamily(), pf->GetStyle(), pf->GetWeight(), false, pf->GetFaceName(), wxFONTENCODING_DEFAULT);
...
int s52plib::RenderT_All(ObjRazRules *rzRules, Rules *rules, bool bTX) {
  ...
  if (text) {
    ...
    if (!text->pFont) {
      ...
      wxFont *specFont = FontMgr::Get().FindOrCreateFont(text->bsize, wxFONTFAMILY_SWISS, wxFONTSTYLE_NORMAL, fontweight, false, wxEmptyString, wxFONTENCODING_DEFAULT);
      ...
      text->pFont = FontMgr::Get().FindOrCreateFont(fontSize, wxFONTFAMILY_SWISS, templateFont->GetStyle(), fontweight_t, false, templateFont->GetFaceName(), wxFONTENCODING_DEFAULT);
  }
...
bool s52plib::RenderHPGL(ObjRazRules *rzRules, Rule *prule, wxPoint &r, float rot_angle, double uScale) {
  ...
  if (rzRules->obj->bIsAton &&
      ...
      if( fabs(rot_angle - 135.0) < 1.) render_angle -= vp_plib.rotation * 180. / PI;
  }
...
bool s52plib::RenderRasterSymbol(ObjRazRules *rzRules, Rule *prule, wxPoint &r, float rot_angle) {
  ...
  if (m_pdc || !texture) {
    ...
    if ((prule->pixelPtr == NULL) || (prule->parm1 != m_colortable_index) ||
      ...
      int nOrgImgWidth = Image.GetWidth();
      int nOrgImgHeight = Image.GetHeight();
...
bool s52plib::RenderSoundingSymbol(ObjRazRules *rzRules, Rule *prule, wxPoint &r, wxColor symColor, float rot_angle) {
  ...
  if (abs(m_SoundingsFontSizeMM - target) > (m_soundFontDelta + .5)) {
    ...
    while ((point_size < 32) && not_done) {
      wxFont *tentativeFont = FontMgr::Get().FindOrCreateFont(point_size, wxFONTFAMILY_SWISS, wxFONTSTYLE_NORMAL, fontWeight, false, fontFacename, wxFONTENCODING_DEFAULT);
...
void s52plib::draw_lc_poly(wxDC *pdc, wxColor &color, int width, wxPoint *ptp,
  ...
  if (pdc) {
    wxPen *pthispen = 0;
    if (!wxThePenList) pthispen = new wxPen(color, width, wxPENSTYLE_SOLID);
    else pthispen = wxThePenList->FindOrCreatePen(color, width, wxPENSTYLE_SOLID);
...
int s52plib::RenderMPS(ObjRazRules *rzRules, Rules *rules) {
  ...
  for (int ip = 0; ip < npt; ip++) {
    ...
    wxColor symColor = GetGlobalColor(_T("SNDG2"));
  ...
  if (m_pdc) {  // DC rendering
    ...
    if ((rules->razRule->pixelPtr == NULL) ||
      ...
      if (fabs(sectr2 - sectr1) != 360)  // not necessary for all-round lights
      {
        ...
        wxPen *pblockpen = 0;
        if (wxThePenList) pblockpen = wxThePenList->FindOrCreatePen(*wxBLACK, 10, wxPENSTYLE_SOLID);
        else pblockpen = new wxPen(*wxBLACK, 10, wxPENSTYLE_SOLID);
      ...
      wxPen *pthispen = 0;
      if (wxThePenList) pthispen = wxThePenList->FindOrCreatePen(color, outline_width * scale_factor, wxPENSTYLE_SOLID);
      else pthispen = new wxPen(color, outline_width * scale_factor, wxPENSTYLE_SOLID);

      mdc.SetPen(*pthispen);

      wxBrush *pthisbrush = 0;
      if (wxTheBrushList) pthisbrush = wxTheBrushList->FindOrCreateBrush(color, wxBRUSHSTYLE_TRANSPARENT);
      else pthisbrush = new wxBrush(color, wxBRUSHSTYLE_TRANSPARENT);
      ...
      if (arc_width) {
        ...
        if (wxThePenList) pthispen = wxThePenList->FindOrCreatePen(colorb, arc_width * scale_factor, wxPENSTYLE_SOLID);
        else pthispen = new wxPen(colorb, arc_width * scale_factor, wxPENSTYLE_SOLID);
        mdc.SetPen(*pthispen);
...
void s52plib::PrepareForRender(VPointCompat *vp) {
  ...
  m_ChartScaleFactorExp = m_pPlatform->GetChartScaleFactorExp();
...
void RenderFromHPGL::SetPen() {
  ...
  if (!wxTheBrushList) {
     brush=new wxBrush(penColor, wxBRUSHSTYLE_SOLID);   
   }else{
  ...
  if (renderToGCDC) {
    ...
    if (wxTheBrushList) brush = wxTheBrushList->FindOrCreateBrush(penColor, wxBRUSHSTYLE_SOLID);
    else brush = new wxBrush(penColor, wxBRUSHSTYLE_SOLID);
	...
	// printf("Render PM\n");
      ...
      if (arguments == _T("0")) {
        do {
          ...
          if (command == _T("AA")) {
            printf("RenderHPGL: PM-AA instruction not implemented.\n");
          }
          if (command == _T("CI")) {
            long radius;
		// printf("Render:HP-CI\n");
