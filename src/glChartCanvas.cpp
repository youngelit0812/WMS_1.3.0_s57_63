...
void glChartCanvas::SetupOpenGL()
{
	...
	printf("OpenGL-> Texture rectangle format: %x\n", g_texture_rectangle_format);

	// VBO??
	g_b_EnableVBO = true;

	if (g_b_EnableVBO)
		printf("OpenGL-> Using Vertexbuffer Objects\n");
	else
		printf("OpenGL-> Vertexbuffer Objects unavailable\n");
	...
	if (m_b_BuiltFBO)
	{
		...
		if (fb_status != GL_FRAMEBUFFER_COMPLETE_EXT)
		{
			printf("    OpenGL-> Framebuffer Incomplete:  %08X\n", fb_status);
			m_b_DisableFBO = true;
			BuildFBO();
		}
	}
	...
	if (m_b_BuiltFBO)
	{
		printf("OpenGL-> Using Framebuffer Objects\n");

		if (m_b_useFBOStencil)
			printf("OpenGL-> Using FBO Stencil buffer\n");
		else
			printf("OpenGL-> FBO Stencil buffer unavailable\n");
	}
	else
		printf("OpenGL-> Framebuffer Objects unavailable\n");

	if (s_b_useStencil)
		printf("OpenGL-> Using Stencil buffer clipping\n");
	else
		printf("OpenGL-> Using Depth buffer clipping\n");

	if (s_b_useScissorTest && s_b_useStencil)
		printf("OpenGL-> Using Scissor Clipping\n");
	...
	printf("OpenGL-> Minimum cartographic line width: %4.1f\n", g_GLMinCartographicLineWidth);
	printf("OpenGL-> Minimum symbol line width: %4.1f\n", g_GLMinSymbolLineWidth);	
...
void glChartCanvas::SetupCompression()
{
	...
	{
		printf("OpenGL-> SSE2 Instruction set not available\n");
		goto no_compression;
	}
#endif
	...
	if (QueryExtension("GL_OES_compressed_ETC1_RGB8_texture"))
	{
		...
		printf("OpenGL-> Using oes etc1 compression\n");
	}
#endif
	...
	if (GL_RGB == g_raster_format)
	{
			 ...
			 QueryExtension("GL_EXT_texture_compression_dxt1")))
		{
			...
			printf("OpenGL-> Using s3tc dxt1 compression\n");
		}
		else if (QueryExtension("GL_3DFX_texture_compression_FXT1"))
		{
			g_raster_format = GL_COMPRESSED_RGB_FXT1_3DFX;

			printf("OpenGL-> Using 3dfx fxt1 compression\n");
		}
		else
		{
			printf("OpenGL-> No Useable compression format found\n");
			goto no_compression;
		}
	}
	...
	printf("OpenGL-> Compressed tile size: %dkb (%d:1)\n", g_tile_size / 1024, g_uncompressed_tile_size / g_tile_size);
	return;
...
no_compression:
	...
	printf("OpenGL-> Not Using compression\n");
}
...
{
	if (!m_pParentCanvas->m_bDisplayGrid)
		return;
	...
	bool minorgrid = fabs(vp.rotation) < 0.0001 && vp.m_projection_type == PROJECTION_MERCATOR;
	...
	wxPen *pen = 0;
	if (!wxThePenList)
		pen = new wxPen(GridColor, g_GLMinSymbolLineWidth, wxPENSTYLE_SOLID);
	else
		pen = wxThePenList->FindOrCreatePen(GridColor, g_GLMinSymbolLineWidth, wxPENSTYLE_SOLID);
	gldc.SetPen(*pen);

	printf("OpenGL Draw Grid.\n");
...
int GLCustomErrorHandler(Display *display)
{
	printf("err code : \n");
...
#ifdef __linux__
	XSetIOErrorHandler(GLCustomErrorHandler);
#endif
	...
	if (m_b_BuiltFBO && !bpost_hilite)
	{
		...
		if (b_newview)
		{
				...
				m_cache_vp.pix_height == VPoint.pix_height)
			{
				...
				accelerated_pan =
					b_whole_pixel && abs(dx) < m_cache_tex_x && abs(dy) < m_cache_tex_y && (abs(dx) > 0 || (abs(dy) > 0));
			}
...
void glChartCanvas::OnEvtPinchGesture(wxQT_PinchGestureEvent &event)
{
	...
	case GestureUpdated:
		...
		else
		{
			...
			if (1 || ((total_zoom_val > 1) && !first_zout))
			{ // Zoom in
				wxPoint pinchPoint = event.GetCenterPoint();
