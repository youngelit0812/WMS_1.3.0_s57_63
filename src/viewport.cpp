...
wxPoint2DDouble ViewPort::GetDoublePixFromLL(double lat, double lon) {
  ...
  switch (m_projection_type) {
    ...
    default:
      printf("GetDoublePixFromLL : unhandled projection\n");
  }
...
void ViewPort::GetLLFromPix(const wxPoint2DDouble &p, double *lat, double *lon) {
  ...
  switch (m_projection_type) {
    ...
    default:
      printf("GetLLFromPix : unhandled projection\n");
  }
...
//
OCPNRegion ViewPort::GetVPRegionIntersect(const OCPNRegion &Region, int nPoints, float *llpoints, int chart_native_scale, wxPoint *ppoints) {
  ...
  int npPoints = 0;
  for (int ip = 0; ip < nPoints; ip++) {
    wxPoint xPoint = GetPixFromLL(pfp[0], pfp[1]);
    if (xPoint.x == INVALID_COORD) continue;

    pp[npPoints++] = xPoint;

    if (valid) {
      poly_x_max = wxMax(poly_x_max, xPoint.x);
      poly_y_max = wxMax(poly_y_max, xPoint.y);
      poly_x_min = wxMin(poly_x_min, xPoint.x);
      poly_y_min = wxMin(poly_y_min, xPoint.y);
    } else {
      poly_x_max = xPoint.x;
      poly_y_max = xPoint.y;
      poly_x_min = xPoint.x;
...
//construct the BoundingBox with llpoints and get left-top point and right-bottom point of the BoundingBox.
//create OCPNRegion from the two points.
//Calculate the intersection the region and this viewport's rv_rect -region.
...
//calc viewport's lat, lon min,max and set it as bbox.
...
//set this Viewport's bbox with parameters.
