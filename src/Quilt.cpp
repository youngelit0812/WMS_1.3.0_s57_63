...
//check chart of active_charttable's index is quiltable.
...
bool Quilt::RenderQuiltRegionViewOnDCNoText(wxMemoryDC &dc, ViewPort &vp, OCPNRegion &chart_region, bool bCSShowFlag, bool bLHShowFlag, bool bBUOYShowFlag, bool bLDESCRShowFlag, bool bAIShowFlag, bool bSlvisShowFlag) {
  return DoRenderQuiltRegionViewOnDC(dc, vp, chart_region, bCSShowFlag, bLHShowFlag, bBUOYShowFlag, bLDESCRShowFlag, bAIShowFlag, bSlvisShowFlag);
}
...
bool Quilt::DoRenderQuiltRegionViewOnDC(wxMemoryDC &dc, ViewPort &vp, OCPNRegion &chart_region, bool bCSShowFlag, bool bLHShowFlag, bool bBUOYShowFlag, bool bLDESCRShowFlag, bool bAIShowFlag, bool bSlvisShowFlag) {
    ...
    if (!chart_region.Empty()) {
      while (chart) {
        ...
        if (pqp->b_Valid) {
          ...
          OCPNRegion get_screen_region = vp.GetVPRegionIntersect(chart_region, get_region, chart->GetNativeScale());
          if (!get_screen_region.Empty()) {
            if (!pqp->b_overlay) {
              if (chart->GetChartType() == CHART_TYPE_CM93COMP) {
                ...
                if (Chs57) {
                  if (Chs57->m_RAZBuilt) {
                    b_chart_rendered = Chs57->RenderRegionViewOnDCNoText(tmp_dc, vp, get_screen_region, bCSShowFlag, bLHShowFlag, bBUOYShowFlag, bLDESCRShowFlag, bAIShowFlag, bSlvisShowFlag);
                  }
                }
              }
