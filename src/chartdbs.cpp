...
ChartTableEntry::ChartTableEntry(ChartBase &theChart, wxString &utf8Path) {
  ...
  char *pUtf8Path = (char *)malloc(strlen(utf8Path.mb_str(wxConvUTF8)) + 1);
  strcpy(pUtf8Path, utf8Path.mb_str(wxConvUTF8));
  pFullPath = pUtf8Path;
    ...
    *pfe++ = fext.WLON;  // LonMin;
	...
	for (int j = 0; j < nAuxPlyEntries; j++) {
		...
		if (nPE > 5 && (LOD_meters > .01)) {
			...
			for (int i = 0; i < nPE; i++) {
				if (DPbuffer[2 * i] > 1000.) {
					*pFloat++ = DPbuffer[2 * i] - 2000.;
					*pFloat++ = DPbuffer[(2 * i) + 1];
				}
			}
...
bool ChartDatabase::Write(const wxString &filePath) {
  ...
  for (int iDir = 0; iDir < cth.GetDirEntries(); iDir++) {
    wxString &sDir = m_chartDirs[iDir];
    int dirlen = sDir.length();
    char s[200];
    strncpy(s, sDir.mb_str(wxConvUTF8), 199);
    s[199] = 0;
    dirlen = strlen(s);
...
wxString ChartDatabase::GetFullChartInfo(ChartBase *pc, int dbIndex,
  ...
  if (1)  // TODO why can't this be cte.GetbValid()?
  {
    ...
    if (pc) {
      line = _(" Name:  ");
      wxString sLongline = pc->GetName();

      wxString tkz;
      if (sLongline.Find(' ') != wxNOT_FOUND)  // assume a proper name
        tkz = _T(" ");
      else
        tkz = _T("/,\\");  // else a file name

      if (sLongline.Len() > target_width) {
...
bool ChartDatabase::Create(ArrayOfCDI& dir_array) {
  ...
  Update(dir_array, true);  // force the update the reload everything
  bValid = true;
//      Explicitly set the version
  m_dbversion = DB_VERSION_CURRENT;
...
bool ChartDatabase::Update(ArrayOfCDI &dir_array, bool bForce) {
  ...
  for (unsigned int j = 0; j < dir_array.GetCount(); j++) {
    ...
    TraverseDirAndAddCharts(dir_info, dir_magic, lbForce);
...
int ChartDatabase::TraverseDirAndAddCharts(ChartDirInfo &dir_info, wxString &dir_magic, bool bForce) {
...
bool ChartDatabase::AddSingleChart(wxString &ChartFullPath,
  ...
  wxString new_magic;
  DetectDirChange(dir_name, _T(""), _T(""), new_magic);
