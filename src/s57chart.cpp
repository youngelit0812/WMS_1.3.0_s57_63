...
bool s57chart::RenderRegionViewOnDCNoText(wxMemoryDC &dc, const ViewPort &VPoint, const OCPNRegion &Region, bool bCSShowFlag, bool bLHShowFlag, bool bBUOYShowFlag, bool bLDESCRShowFlag, bool bAIShowFlag, bool bSlvisShowFlag) {
  if (!m_RAZBuilt) return false;
...
bool s57chart::DoRenderViewOnDC(wxMemoryDC &dc, const ViewPort &VPoint, RenderTypeEnum option, bool force_new_view, bool bCSShowFlag, bool bLHShowFlag, bool bBUOYShowFlag, bool bLDESCRShowFlag, bool bAIShowFlag, bool bSlvisShowFlag) {
  bool bnewview = false;
  wxPoint rul, rlr;
  bool bNewVP = false;
  ...
  if (bNewVP && (NULL != pDIB) && !rgn_last.IsEmpty()) {
    ...
    while (upd.HaveRects()) {
      ...
      DCRenderRect(dc, temp_vp, &rect, bCSShowFlag, bLHShowFlag, bBUOYShowFlag, bLDESCRShowFlag, bAIShowFlag, bSlvisShowFlag);
    ...
    DCRenderRect(dc, VPoint, &full_rect, bCSShowFlag, bLHShowFlag, bBUOYShowFlag, bLDESCRShowFlag, bAIShowFlag, bSlvisShowFlag);
...
int s57chart::DCRenderRect(wxMemoryDC &dcinput, const ViewPort &vp, wxRect *rect, bool bCSShowFlag, bool bLHShowFlag, bool bBUOYShowFlag, bool bLDESCRShowFlag, bool bAIShowFlag, bool bSlvisShowFlag) {
  int i;
  ObjRazRules *top;
  ObjRazRules *crnt;
...
//      Convert the Private render canvas into a bitmap
  ...
  DCRenderLPB(dcinput, vp, rect, bCSShowFlag, bLHShowFlag, bBUOYShowFlag, bLDESCRShowFlag, bAIShowFlag, bSlvisShowFlag);

  return 1;
}

bool s57chart::DCRenderLPB(wxMemoryDC &dcinput, const ViewPort &vp, wxRect *rect, bool bCSShowFlag, bool bLHShowFlag, bool bBUOYShowFlag, bool bLDESCRShowFlag, bool bAIShowFlag, bool bSlvisShowFlag) {
  int i;
  ObjRazRules *top;
  ObjRazRules *crnt;
  ViewPort tvp = vp;  // undo const  TODO fix this in PLIB
      ...
      for (int iff = 0; iff < retval + 1; iff++) {
            ...
            (flen > 25))  // a valid update file or base file
        {
          ...
          if (!chain_broken_mssage_shown) {
			  printf("S57 Cell Update chain incomplete.\nENC features may be \nincomplete or inaccurate.\nCheck the logfile for details.\n");
...
void s57_DrawExtendedLightSectorsGL(ocpnDC & dc, ViewPort & viewport, std::vector<s57Sector_t> & sectorlegs) {
	float rangeScale = 0.0;
