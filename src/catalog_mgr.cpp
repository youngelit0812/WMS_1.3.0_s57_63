...
protected:
  ...
  struct ActiveCatalogGrid : public wxPanel, public Helpers {
    ActiveCatalogGrid(wxWindow* parent) : wxPanel(parent), Helpers(this) {
    ...
    void UpdateVersion(wxSizer* grid, CatalogData data, size_t ix) {
      auto version =
          dynamic_cast<wxStaticText*>(grid->GetItem(ix)->GetWindow());
      version->SetLabel(data.version);
      auto date =
          dynamic_cast<wxStaticText*>(grid->GetItem(ix + 1)->GetWindow());
    ...
    void ReloadAvailableVersion() {
      ...
      if (message != "") {
        wxMessageBox(message, _("Catalog update problem"), wxICON_ERROR);
      } else {
        UpdateVersions();
      }
    }
  ...
  struct Buttons : public wxPanel, public Helpers {
    ...
    void clearUrl() { m_parent->m_url_edit->clear(); }
  ...
  struct UrlEdit : public wxPanel, public Helpers {
    UrlEdit(wxWindow* parent) : wxPanel(parent), Helpers(this) {
      ...
      auto flags = wxSizerFlags().Align(wxALIGN_CENTER_VERTICAL);
      url_location->Add(staticText(_("Custom catalog URL: ")), flags);
...
public:
  ...
  CatalogLoad(wxWindow* parent, bool use_latest = false)
    ...
    Bind(CATALOG_DL_DONE, [=](wxCommandEvent& ev) { m_grid->CellDone(ev, 5); });
    Bind(CATALOG_PARSE_DONE, [=](wxCommandEvent& ev) { workerDone(ev); });
  }
  ...
  void Worker() {
    ...
    if (status == catalog_status::OK) {
      PostEvent(CATALOG_PARSE_DONE, catalog_status::OK_MSG,
                catalog->LatestCatalogData().version);
    } else {
      PostEvent(CATALOG_PARSE_DONE, status, catalog->LastErrorMsg());
    }
  }
  ...
  struct Buttons : public wxPanel {
    ...
    void ActivateOk() { m_ok->Enable(true); }
...
AdvancedCatalogDialog::AdvancedCatalogDialog(wxWindow* parent)
    : wxFrame(parent, wxID_ANY, _("Catalog Manager"), wxDefaultPosition,
...
SimpleCatalogDialog::SimpleCatalogDialog(wxWindow* parent)
    : wxDialog(parent, wxID_ANY, _("Catalog Manager"), wxDefaultPosition,
               wxDefaultSize, wxDEFAULT_DIALOG_STYLE | wxRESIZE_BORDER) {
  auto vbox = new wxBoxSizer(wxHORIZONTAL);
  vbox->Add(new catalog_mgr::CatalogLoad(this, true), wxSizerFlags(1).Expand());
