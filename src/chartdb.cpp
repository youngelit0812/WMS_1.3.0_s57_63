...
void ChartStack::AddChart(int db_add) {
  ...
  while ((id < j)) {
    if (GetDBIndex(id) == -1) {
      int jd = id + 1;
      while (jd < j) {
        int new_db_index = GetDBIndex(jd);
        SetDBIndex(jd - 1, new_db_index);
        jd++;
      }
...
ChartBase *ChartDB::GetChart(const wxChar *theFilePath,
  ...
  if (chartExt == wxT("XZ")) {
    ...
    wxFileName fnLocal(npath);
    chartExt = fnLocal.GetExt().Upper();
  }
...
int ChartDB::BuildChartStack(ChartStack *cstk, float lat, float lon,
  ...
  int id = 0;
  while ((id < j)) {//check indexs stored in DBIndex of chartstack, if it's -1, remove it from DBIndex.
    if (cstk->GetDBIndex(id) == -1) {
      int jd = id + 1;
      while (jd < j) {
...
bool ChartDB::IsChartInGroup(const int db_index, const int group) {
  ChartTableEntry *pChartTableEntry = (ChartTableEntry *)&GetChartTableEntry(db_index);

  //    Check to see if the candidate chart is in the designated group
  bool b_in_group = false;
  if (group > 0) {
    for (unsigned int ig = 0; ig < pChartTableEntry->GetGroupArray().size(); ig++) {
      if (group == pChartTableEntry->GetGroupArray()[ig]) {
        b_in_group = true;
        break;
      }
    }
...
//loop active_charttable and check every charttableentry, if anyone's chart type isn't MBTILES, true
...
//      Check the lat,lon is located in the chart who's db index is "index".
//      If the chart contains Auxs, the lat,lon must be in the one of the Auxs.
//-------------------------------------------------------------------
bool ChartDB::CheckPositionWithinChart(int index, float lat, float lon) {
  const ChartTableEntry *pChartTableEntry = &GetChartTableEntry(index);

  //    First check on rough Bounding box

  if ((lat <= pChartTableEntry->GetLatMax()) && (lat >= pChartTableEntry->GetLatMin()) &&
      (lon >= pChartTableEntry->GetLonMin()) && (lon <= pChartTableEntry->GetLonMax())) {
    //    Double check on Primary Ply points polygon

    //check the lon and lat are in ChartTableEntry's polygon.
    bool bInside = G_FloatPtInPolygon((MyFlPoint *)pChartTableEntry->GetpPlyTable(), pChartTableEntry->GetnPlyEntries(), lon, lat);

    if (bInside) {
      if (pChartTableEntry->GetnAuxPlyEntries()) {
        for (int k = 0; k < pChartTableEntry->GetnAuxPlyEntries(); k++) {
          bool bAuxInside =
              G_FloatPtInPolygon((MyFlPoint *)pChartTableEntry->GetpAuxPlyTableEntry(k), pChartTableEntry->GetAuxCntTableEntry(k), lon, lat);
      ...
      if (!m_b_locked) {
        ...
        if (g_memCacheLimit) {
		  ...
		  if ((mem_used > g_memCacheLimit * 8 / 10) &&
            ...
            while (1) {
              CacheEntry *pChartEntry = FindOldestDeleteCandidate(true);
              if (pChartEntry == 0) break;  // no possible delete candidate

              // purge texture cache, really need memory here
              DeleteCacheEntry(pChartEntry, true, msgRemove);
        ...
        else  // Use n chart cache policy, if memory-limit  policy is not used
        {
          ...
          //      needed
          unsigned int nCacheCount = pChartCache->GetCount();
          if (nCacheCount > (unsigned int)g_nCacheLimit && nCacheCount > 2) {
            wxString msg(_T("Removing oldest chart from cache: "));
            while (nCacheCount > (unsigned int)g_nCacheLimit) {
              CacheEntry *pChartEntry = FindOldestDeleteCandidate(true);
              if (pChartEntry == 0) break;

              DeleteCacheEntry(pChartEntry, true, msg);
              nCacheCount--;
            }
          }
        }
...
bool ChartDB::CheckExclusiveTileGroup(int canvasIndex) {
  ...
  // Check the group for anything other than MBTiles... //check any chart not MBTiles exists in active_charttable, then true
