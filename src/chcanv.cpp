...
bool ChartCanvas::DoCanvasUpdate(void)
{
	...
	{ // Bogus Lat, Lon?
		...
		if (m_singleChart)
			if (m_singleChart->GetChartType() != CHART_TYPE_DUMMY)
				bNewChart = true;
	...
	if (!ChartData->EqualStacks(&WorkStack, m_pCurrentStack))
	{
		...
		if (tEntry != -1)
		{ // m_singleChart is in the new stack
		...
		if (NULL != m_singleChart)
		{
			...
			{ // otherwise, match scale if elected.
				double proposed_scale_onscreen;

				if (m_bFollow)
				{ // autoset the scale only if in autofollow
				...
				if (m_bFollow)
				{ // bounds-check the scale only if in autofollow
					proposed_scale_onscreen =
...
int ChartCanvas::GetCanvasChartNativeScale()
{
	...
	{
		if (m_singleChart)
			ret = m_singleChart->GetNativeScale();
	}
	else
		...
		if (pc)
		{
			...
			else
			{
				...
				if (!brender_ok)
				{
					...
					if (brender_ok)
					{ // found a better reference chart
						int new_db_index =
							m_pQuilt->GetExtendedStackIndexArray()[target_stack_index];
	...
	bool b_adjust, bool b_refresh)
{
	...
	if (!VPoint.b_quilt && m_singleChart)
	{
		...
		if (b_adjust)
			m_singleChart->AdjustVP(last_vp, VPoint);
...
void ChartCanvas::GridDraw(ocpnDC& dc)
{
	...
	if (dlon < 0.0)
	{ // concider datum border at 180 degrees longitude
		dlon = dlon + 360.0;
	}
	...
	if (dbIndex >= 0)
	{
		...
		if (!m_pCIWin->IsShown() || (m_pCIWin->dbIndex != dbIndex))
		{
			...
			if ((p.x + m_pCIWin->GetWinSize().x) > (m_canvas_width / GetContentScaleFactor()))
				p.x = ((m_canvas_width / GetContentScaleFactor()) - m_pCIWin->GetWinSize().x) / 2; // centered

			p.y =
				(m_canvas_height - m_Piano->GetHeight()) / GetContentScaleFactor() - 4 - m_pCIWin->GetWinSize().y;
...
void ChartCanvas::DrawCanvasData(LLBBox& llbBox, int nWidth, int nHeight, std::vector<int>& vnLayers, std::string& sIMGFilePath, bool bPNGFlag)
{
	...
	if (temp_dc.IsOk())
	{ // process background except the ENC data's region (draw water and rotate)
...
void ChartCanvas::SetAISCanvasDisplayStyle(int StyleIndx)
{
	...
	if (StyleIndx == -1)
	{ // -1 means coming from toolbar button
...
void ChartCanvas::RenderAlertMessage(wxDC& dc, const ViewPort& vp)
{
	if (!GetAlertString().IsEmpty())
	{
		wxFont* pfont = 0;
		if (wxTheFontList)
			pfont = wxTheFontList->FindOrCreateFont(10, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL);
		else
			pfont = new wxFont(10, wxFONTFAMILY_DEFAULT, wxFONTSTYLE_NORMAL, wxFONTWEIGHT_NORMAL, false, wxEmptyString, wxFONTENCODING_DEFAULT);
