        ...
        if (!high_res_map) {
          ...
          for (unsigned int pi = 0; pi < poly1.size(); pi++) {
            ...
            for (unsigned int pj = 0; pj < c.size(); pj++) {
              double dLon = c[pj].x, dLat = c[pj].y;
              ...
              if (lx == dLon && ly == dLat) continue;

              int statex = dLon < minlon ? -1 : dLon > maxlon ? 1 : 0;
              int statey = dLat < minlat ? -1 : dLat > maxlat ? 1 : 0;

              if ((!statex || lstatex != statex) && (!statey || lstatey != statey)) {
                high_res_map->push_back(wxLineF(lx, ly, dLon, dLat));
              }
...
//-------------------------------------------------------------------------
void GshhsPolyReader::drawGshhsPolyMapPlain(ocpnDC &pnt, ViewPort &vp, wxColor const &seaColor, wxColor const &landColor) {
  if (!fpoly) return;
